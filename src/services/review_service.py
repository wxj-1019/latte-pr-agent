...
def _create_provider(review):
    """Create Git provider based on review platform."""
    if review.platform == "github":
        return GitProviderFactory.from_pr_info(
            platform="github",
...
async def _run_review_core(
    session: AsyncSession,
    review,
    provider,
) -> None:
    """Core review pipeline logic.  Runs inside a transaction managed by the caller."""
    review_repo = ReviewRepository(session)
    await review_repo.update_status(review.id, "running")

    diff_content = await provider.get_diff_content()
    logger.info("Review %s: fetched diff (%s chars)", review.id, len(diff_content))
    ...
    if repo_path:
        try:
            ...
            logger.info("Review %s: project config loaded", review.id)
        except Exception as exc:
            logger.warning("Review %s: project config load failed: %s", review.id, exc, exc_info=True)
    ...
    if repo_path:
        try:
            ...
            logger.info("Review %s: dependency graph updated", review.id)
        except Exception as exc:
            logger.warning("Review %s: dependency graph build failed: %s", review.id, exc, exc_info=True)
    ...
    await review_repo.update_status(
        review.id, "completed",
        risk_level=final_risk_level if not result.get("degraded") else None
    )
